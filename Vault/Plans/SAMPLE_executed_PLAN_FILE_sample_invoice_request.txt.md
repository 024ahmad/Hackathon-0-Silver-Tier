...
- [x] Step 1: Validate task metadata (type=file_drop, priority=normal, needs_human_approval=false)
- [x] Step 2: Locate and read related artifact (Needs_Action/FILE_sample_invoice_request.txt)
- [x] Step 3: Analyse artifact content — extract client, amount, due date, service type
- [x] Step 4: Check if human approval is required (needs_human_approval: false — proceed, flag financial risk)
- [x] Step 5: Prepare result output document with summary, extracted fields, and next steps
- [x] Step 6: Mark plan ready for execution
...
- **Execution note**: Plan executed by `execute_plan` skill. Result document
  written to Vault/Done/. Task and artifact moved to Done/Archive respectively.
